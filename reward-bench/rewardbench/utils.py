...
def prepare_dialogue_from_tokenizer(
    ...
    if all(k in example.keys() for k in ("chosen", "rejected")):
        ...
        # single turn
            ...
            chosen_text = tokenizer.apply_chat_template([chosen_response[-1]], tokenize=False, add_generation_prompt=False)
            rejected_text = tokenizer.apply_chat_template([rejected_response[-1]], tokenize=False, add_generation_prompt=False)
            ...
            if 'phi' in model_type.lower():
                if chosen_text.endswith(tokenizer.eos_token):
                    chosen_text = chosen_text[:-len(tokenizer.eos_token)].strip()
                    chosen_text = chosen_text.replace("<|assistant|>\n", "")
                if rejected_text.endswith(tokenizer.eos_token):
                    rejected_text = rejected_text[:-len(tokenizer.eos_token)].strip()
                    rejected_text = rejected_text.replace("<|assistant|>\n", "")
                if prompt_text.endswith(tokenizer.eos_token):
                    prompt_text = prompt_text[:-len(tokenizer.eos_token)]
                    prompt_text = prompt_text + "<|assistant|>\n"

            elif model_type == "rlhflow_llama_3_sft_8b_v2":
            ...
            elif model_type == 'meta_llama_3_1_instruct_8b':
                marker = '<|eot_id|>'
            ...
            example["chosen_text_only"] = chosen_text
            example["rejected_text_only"] = rejected_text
    elif ift:
