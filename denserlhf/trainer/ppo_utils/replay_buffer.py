...
def split_experience_batch(experience: Experience) -> List[BufferItem]:
    ...
    for k, v in experience.info.items():
        if v.dim() > 1:  # If the item is a matrix or higher-dimensional tensor
            vals = list(torch.unbind(v, dim=0))  # Unbind along the first dimension
            for i, vv in enumerate(vals):
                batch_kwargs[i]["info"][k] = vv
        else:  # Assume scalar or single-dimension tensor for each batch item
            vals = torch.unbind(v)
            assert batch_size == len(vals)
            for i, vv in enumerate(vals):
                batch_kwargs[i]["info"][k] = vv.item() if vv.numel() == 1 else vv
...
def make_experience_batch(items: List[BufferItem], packing_samples=False) -> Experience:
    ...
    for key in items[0].info.keys():
        # Handle variable tensor data from 'info', respecting the original data shape
        if isinstance(items[0].info[key], torch.Tensor) and items[0].info[key].dim() > 0:
            # Find the maximum length of tensors in this batch
            max_len = max(item.info[key].size(0) for item in items)
            # Pad each tensor to this max length and then stack
            padded_vals = [F.pad(item.info[key], (0, max_len - item.info[key].size(0))) for item in items]
            vals = torch.stack(padded_vals)
        else:
            vals = [item.info[key] for item in items]
            vals = torch.tensor(vals) if isinstance(vals[0], (int, float, bool)) else torch.stack(vals)
