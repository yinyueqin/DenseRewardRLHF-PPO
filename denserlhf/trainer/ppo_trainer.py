...
from denserlhf.models import Actor, GPTLMLoss, PolicyLoss, ValueLoss
from denserlhf.models.utils import masked_mean
...
class PPOTrainer(ABC):
    ...
    def __init__(
        ...
        entropy_threshold: float = 2,
        num_thresholds: int = 1,
        ppo_reward_type: str = "segment_normalization",
        agg_func: str = "avg",
        segment_method: str = "peak",
        model_type: str = "phi3",
        reward_fit_dataset: str = "preference700k",
        **generate_kwargs,
    ) -> None:
        assert (
        ...
        self.generate_kwargs["tokenizer"] = tokenizer
        ...
        self.experience_maker = NaiveExperienceMaker(
            ...
            self.num_thresholds,
            self.ppo_reward_type,
            self.agg_func,
            self.segment_method,
            self.reward_fit_dataset,
            self.model_type
        )
    ...
    def fit(
        ...
        self.strategy.print("steps: {}, start_episode: {}, consumed_samples: {}".format(steps, start_episode, consumed_samples))
        self.strategy.args.print_index = steps
        self.strategy.print("\nprint_index: ", self.strategy.args.print_index)
        ...
        for episode in range(start_episode, args.num_episodes):
            ...
            for rand_prompts in self.prompts_dataloader:
                ...
                self.strategy.print(f"In generate experience function, global step: {steps}, experience.sequences: {experience.sequences.shape}")
                ...
                if steps % update_timesteps == 0:
                    ...
                    self.strategy.print(f"\nIteration time for parameter update: {time.time() - start_iteration}")
    ...
    def ppo_train(self, global_step=0):
        ...
        for epoch in range(self.max_epochs):
            ...
            self.strategy.print(f'\nStart ppo training in global step: {global_step} with replay buffer size: {len(self.replay_buffer)}')
            for i, experience in enumerate(pbar):
                experience.to_device(device)
                self.strategy.print(f'\nIn ppo_train function: the current progress:', i, '/', len(dataloader))
                status = self.training_step(experience, global_step)
                ...
                if "policy_loss" in status:
                    short_status = {
                        ...
                        "slen": status["avg_segment_lengths"],
                    }
    ...
    def training_step_actor(self, experience: Experience) -> Dict[str, float]:
        ...
        # loss function
        actor_loss, ratio, advantage = self.actor_loss_fn(
            action_log_probs,
        ...
        # status
        status = {"policy_loss": actor_loss.item(), "actor_lr": self.actor_scheduler.get_last_lr()[0], 'ratio': ratio.item(), 'advantage': advantage.item()}
        if self.pretrain_dataloader is not None:
        ...
        status["actor_grad_norm"] = 0.0 if grad_norm is None else grad_norm
    ...
    def training_step_critic(self, experience: Experience) -> Dict[str, float]:
        ...
        grad_norm = self.critic.get_global_grad_norm()
        grad_norm = grad_norm.item() if torch.is_tensor(grad_norm) else grad_norm
        status["critic_grad_norm"] = 0.0 if grad_norm is None else grad_norm
    ...
    def save_logs_and_checkpoints(self, args, global_step, step_bar, logs_dict={}, client_states={}):
        ...
        if global_step % args.save_steps == 0:
            ...
            self.strategy.save_model(self.actor.model,
                                        self.tokenizer,
                                        os.path.join(args.save_path, f"checkpoint-{global_step}"))
