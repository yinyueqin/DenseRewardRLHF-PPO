...
def pad_right(sequence, max_length, padding_value):
    padding = [padding_value] * (max_length - len(sequence))
    return torch.tensor(sequence + padding)

def retry(model_name_or_path, retries=3, delay=30):
...
def get_llm_for_sequence_regression(
    ...
    **kwargs,
    ...
    Args:
        ...
        reward_mean (float, optional): Mean value for reward normalization. Defaults to -100.
        reward_std (float, optional): Standard deviation value for reward normalization. Defaults to -100.
    ...
    config = retry(model_name_or_path)
    ...
    if model_type == "reward":
        cls_class = _get_reward_model(base_pretrained_class, base_class, value_head_prefix, packing_samples, reward_mean, reward_std)
    else:
...
def _get_reward_model(base_pretrained_model, base_llm_model, value_head_prefix="value_head", packing_samples=False, reward_mean=-100, reward_std=-100):
    class RewardModel(base_pretrained_model):
        ...
        def __init__(self, config: AutoConfig):
            ...
            if reward_mean != -100 and reward_std != -100:
                self.mean[0] = reward_mean
                self.std[0] = reward_std
                print(f"manual set mean/std: {reward_mean}/{reward_std}")
            elif hasattr(config, "mean"):
                self.mean[0] = config.mean
                self.std[0] = config.std
                print(f"reward model has config mean/std: {config.mean}/{config.std}")
            else:
                print("reward model does not have mean/std in config")
        ...
        def forward(
            ...
            return_output=False,
            return_every_step_reward=False,
...
def _get_critic_model(base_pretrained_model, base_llm_model, value_head_prefix="value_head", packing_samples=False):
    class CriticModel(base_pretrained_model):
        ...
        def __init__(self, config: AutoConfig):
            ...
            print('\ninitialize critic model from sft model, set critic model mean and std as 0 and 1')
